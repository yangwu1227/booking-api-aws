...
The following secrets must be configured in the GitHub repository to enable secure and successful deployment and testing:
...
1. **AWS_ECS_SECURITY_GROUP_ID**: The ID of the security group assigned to the AWS ECS service. This security group controls inbound and outbound traffic, ensuring only authorized access for the application containers and related services like database migrations.

2. **AWS_PRIVATE_SUBNET_1_ID** & **AWS_PRIVATE_SUBNET_2_ID**: The IDs of the private subnets used for deploying ECS tasks. This subnet provides a secure, isolated environment for running application containers, handling database migrations, and managing user credential creation and updates.

This setup allows the application to run migrations, manage credentials, and operate as standalone containers within a secure, private network.

### GitHub Actions Integration

4. **AWS_GITHUB_ACTIONS_ROLE_ARN**: The Amazon Resource Name (ARN) for the IAM role that allows GitHub Actions to securely interact with AWS services using OpenID Connect (OIDC). This role is essential for deploying infrastructure and managing ECS services from GitHub Actions workflows. For more information on configuring this role, refer to the [GitHub documentation](https://docs.github.com/en/actions/security-for-github-actions/security-hardening-your-deployments/configuring-openid-connect-in-amazon-web-services).

### Testing and Code Coverage

5. **AWS_REGION**: The AWS region where all resources (e.g., ECS, RDS, subnets) are hosted. This ensures that deployments and container operations are executed in the correct regional context.

6. **CODECOV_TOKEN**: The token required for integrating Codecov into the CI/CD pipeline to report test coverage results. This token allows Codecov to receive test reports securely and integrate them with GitHub Actions. For more details, see the [Codecov GitHub Action documentation](https://github.com/codecov/codecov-action).
